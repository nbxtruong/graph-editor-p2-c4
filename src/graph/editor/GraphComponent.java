...
public class GraphComponent extends JComponent implements MouseInputListener {
	...
	public void setShapeType(String type) {
		if (shapeType != CIRCLE && shapeType != SQUARE && shapeType != ARC)
	...
	private RectangularShape createShape(int x, int y) {
		...
		if (getShape(x, y) != null)
			throw new IllegalArgumentException();
		if (shapeType == CIRCLE)
			rs = new Ellipse2D.Double(x - RADIUS, y - RADIUS, 2 * RADIUS, 2 * RADIUS);
		else if (shapeType == SQUARE)
			rs = new Rectangle2D.Double(x - RADIUS, y - RADIUS, 2 * RADIUS, 2 * RADIUS);
		else if (shapeType == ARC)
			rs = new Arc2D.Double(x - RADIUS, y - RADIUS, 2 * RADIUS, 2 * RADIUS, 30, 300, Arc2D.PIE);
		shapes.add(rs);
		Random r = new Random();
